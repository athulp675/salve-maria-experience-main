  ...
  var resp_related = ZOHO.CRM.API.searchRecord({
    ...
    if (related_data != undefined) {
      ...
      related_data.forEach((element) => {
        ...
        cell2.innerHTML = element.Job_Title;
        // var exactdate=GetFormattedDate(element.From)
...
function handleDate() {
  ...
  //       console.log(related_data)
  //       var last_experience_date = related_data[0].From;
  //       document.getElementById("to").value = `${last_experience_date}`;
  //   }
  //   else{
  //     var date = new Date();
